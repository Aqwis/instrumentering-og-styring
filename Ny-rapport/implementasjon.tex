...
Ved prosjektets start var siste stabile utgivelse av OpenCV versjon 2.4.8, som ikke var støttet programmering i Matlab. For å bruke Matlab og OpenCV sammen gjensto det to alternativer. Versjon 3.0 av OpenCV, som var under utvikling, støttet Matlab. I tillegg fantes det et tredjeparts kompatibilitetslag, \emph{mexopencv}, som gjorde det mulig å programmere med OpenCV i Matlab. Begge muligheter ble undersøkt. \emph{mexopencv} skapte problemer ved kompileringen på med de ulike kombinasjonene av operativsystemene, og de ulike Matlab-versjon som gruppen hadde inne allerede. Videre hadde \emph{mexopencv} problemer med grafikkortene på de ulike maskinene fordi det ikke var støtte for disse. Utviklingsversjonen av OpenCV, på sin side, ville overhodet ikke kompilere. Antakelig skyldes dette at kodebasen for denne versjonen var under rask utvikling og dermed ustabil.
...
Kameraet fremga et bilde i fargespekteret RGB, som er lite egnet til å separere ulike farger fra hverandre. Valget ble dermed å benytte fargespekteret HSV, som er hyppig brukt innen bildeprosessering. Hvert bilde ble konvertert i videoen fra RGB, som vist i figur [\ref{fig:firstiterationrgb}], til HSV, som vist i figur [\ref{fig:firstiterationhsv}], før bildene ble prosessert videre.
...
Neste steg i prosessen var å velge et objekt i bildet som skulle spores. Definisjonen av objekt menes her et sammenhengende hvitt område i det binære bildet. Den algoritmen som ble skrevet valgte ut det største objektet og returnerte objektets midtpunkt. Dersom det var for mye støy i bildet, det vil si for mange ulike objekter, returnerte algoritmen at objektet ikke ble funnet.

\subsubsection{Smoothing}

Objektet ble i det første steget fanget opp, men som presentert i figur [\ref{fig:firstiterationbinary}] er det mye støy i bildet. En smoothing-algoritme ble derfor implementert. Algoritmen går over det binære bildet og fjerner mindre ansamlinger med punkter og fremhever de større. Dette førte til et renere bilde, med færre og tydeligere objekter. Resultatet av smoothingen er presentert i figur [\ref{fig:seconditerationbinary}].
...
Smoothingen fremhevet de store objektene i bildet. Dette gjorde det betydelig lettere å fange opp det bestemte, gitt riktig innstilling av fargefiltrene, ettersom små objekter i bildet med samme farge som objektet ble eliminert.
